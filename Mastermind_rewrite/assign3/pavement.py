...
@task
def test():
    sh('python -m coverage run --source src --omit src/airport_sorter_UI.py,src/AirportsData.py -m unittest discover -s test')
...
@task
def run():
    sh("python -m src.airport_sorter_UI < input/input.txt")
